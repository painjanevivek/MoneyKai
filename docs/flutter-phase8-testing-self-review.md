...
- Encrypted backup restore validates decrypted contents before reset, rejects wrong passwords/unsupported versions/malformed payloads/invalid users/email whitespace/invalid money values/invalid budget shapes, preserves existing local data on invalid restore failure, resets only the MoneyKai namespace for valid restores, restores user, transactions, and budget, and restores theme settings when present.
...
- Add/edit/delete transaction flow works, including editing restored records with non-default category/payment metadata, historical dates, and future dates.
- Transactions group by month, filter by category, and keep the category fallback cleared after deleting and later re-adding the last matching category.
...
- Local transaction repository, export, and encrypted restore tests now stress a 1,200-row transaction history, verifying newest-first restore order, full JSON export without truncating rows, and encrypted restore transaction counts.
...
The collector writes Android screenshots through binary stdout redirection so captured PNG evidence is not re-encoded by Windows PowerShell, records size and SHA-256 metadata for captured evidence in the summary, and fails if an `adb` command fails, Android launch status/timing is incomplete, default cold-start limits are exceeded (`TotalTime <= 5000 ms`, `WaitTime <= 6000 ms`), the hierarchy does not include the MoneyKai package, or device/PNG screenshot evidence is missing or invalid.
