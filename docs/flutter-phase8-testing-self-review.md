...
- `.\tool\audit_android_release.ps1` passed for current unsigned inspection artifacts, compiled APK identity/version/SDK/label/launch/ARM64 ABI checks, release manifest hardening checks, release AAB structure checks, and restricted-permission checks.
...
- Release signing no longer silently uses the debug key; release builds are unsigned unless all upload-key env vars are provided, and the release audit fails if no-env artifacts are unexpectedly signed.
