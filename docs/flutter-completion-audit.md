...
| Physical Android device QA | Not complete | No physical Android device is connected; `apps\MoneyKai-flutter\tool\collect_android_runtime_qa.ps1 -RequirePhysical` is ready to fail on `adb` command errors, validate launch success/timing, require MoneyKai hierarchy package evidence, and collect binary PNG screenshot plus device evidence once hardware is available. |
