...
| Android permission/privacy audit | Complete for current artifacts | Phase 6 docs and `.\tool\audit_android_release.ps1` verify compiled APK identity/version/SDK/label/launch/ARM64 ABI, release manifest hardening, exported-component exposure, Android backup/data-extraction opt-out, and no restricted SMS/storage/contact/camera/mic/location permissions. |
...
| Security/privacy self-review | Partially complete | Permissions, exported-component exposure, Android backup/data-extraction opt-out, local-only boundaries, enforced storage namespace, reset namespace, plaintext export contents, encrypted backups, local diagnostics clearing, and privacy copy are documented; local storage is not encrypted and real auth/backend sync remain future work. |
