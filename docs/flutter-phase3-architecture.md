...
| Local persistence | `shared_preferences` behind a MoneyKai storage service | Small dependency surface for early local state, with enforced `moneykai.*` key access, schema version metadata, and a namespace reset boundary. More complex transaction storage can move to SQLite/Drift/Isar only when query complexity justifies it. |
