...
- Minimum verified release baseline is `main` at `1d8a067` (`1d8a0679640cc80452ca2f8bb664f3952b411e10`).
- A documented release baseline is intentionally an ancestor of the checked-out commit, not the commit currently being edited. This avoids an impossible self-referential commit-hash requirement while keeping the exact artifact hash and source commit in the final handoff capture.
- Expo v56 docs were checked before mobile release updates on 2026-09-18: `https://docs.expo.dev/versions/v56.0.0/`. The current mobile baseline matches the expected Expo/RN generation used by this repo: React Native `0.85.3`, React `19.2.3`, Node `>=22.13.0`, Android compile SDK `36`, target SDK `36`.
...
- `production`: Play AAB candidate. Builds `android.buildType=app-bundle`, uses `autoIncrement`, disables dev-client metadata, SMS, notification capture, contacts, Gmail/PDF imports, wealth/AI surfaces, and remote diagnostics. It is intended for EAS-managed Android credentials unless intentionally changed.
...
`npm run release:handoff-baseline` verifies that every documented release baseline is an ancestor of the checked-out `HEAD`. Keep it green before starting the next production AAB build; the artifact-capture command records the exact source commit used for the upload candidate.
...
- Play-safe production builds must not include SMS, contacts, legacy external-storage, microphone, or notification-listener manifest entries.
...
- Notification listener capture is excluded from the public Play build at the Gradle dependency boundary and is also removed through the release manifest as defense in depth.

## Remaining blockers before Play internal handoff

- Produce a fresh production AAB from the current clean `main` descendant of the verified baseline.
...
- Record artifact path, SHA-256, EAS build URL/build ID, signing expectation, device smoke result, tester group, and cloud-sync/backup visibility.
- Smoke the signed artifact on a physical Android device, including launch, sign-in, expense entry, cloud sync, backup preview/restore, notification opt-in, and the in-app testing report bundle. Confirm the capture controls are absent.
