...
Target: **Play Console internal testing**, then closed testing; not public production launch.
...
| Field | Value |
| --- | --- |
| Minimum verified baseline | `1d8a067` (`1d8a0679640cc80452ca2f8bb664f3952b411e10`) |
...
| `1d8a067` | Minimum verified source baseline before the cloud-sync minimal Play release hardening. |

## Build split

- **MoneyKai**: Play Store-safe internal testing build. Must not contain SMS, contacts, legacy storage, microphone, or notification-listener capture capabilities.
...
| --- | --- |
| Play-safe internal AAB | Pending fresh production AAB from a clean `main` descendant of the verified baseline |
...
- Notification listener capture is intentionally excluded from the public Play build; verify that its controls and manifest service are absent.
- Original MoneyKai SMS validation needs real incoming carrier/bank SMS and remains local/internal only.
- Remote Sentry reporting and diagnostic uploads are disabled in the public Play profile; no diagnostics account setup is required for this candidate.
...
- For crashes, freezes, failed auth, failed navigation, cloud-sync/backup issues, or incorrect totals, testers should include the in-app testing report bundle plus the fields below.
...
- Whether optional app notifications were granted, denied, revoked, or left disabled.
- Confirmation that notification-listener and automatic-capture controls are absent from the public Play build.
...
| Source baseline | `1d8a067` / `1d8a0679640cc80452ca2f8bb664f3952b411e10` |
| Branch | `main` |
| App version | `1.0.1` |
| Android package | `com.moneykai.mobile` |
...
| Optional app notifications tested | Pending |
| Automatic capture excluded | Pending exact artifact verification |
...
| Remote diagnostics disabled | Pending exact artifact verification |
