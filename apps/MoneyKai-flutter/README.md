...
The release audit script also fails on partial signing env, unexpected signed artifacts when no upload-key env is set, compiled APK identity/version/SDK/label/launch/ABI drift, release debuggability or cleartext-traffic drift, missing AAB base-module entries, restricted permissions, missing artifacts, or unsigned artifacts when `-RequireSigned` is used.
