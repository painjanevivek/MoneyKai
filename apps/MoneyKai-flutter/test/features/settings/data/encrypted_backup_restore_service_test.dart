...
void main() {
  ...
  test(
    'rejects encrypted backup with malformed budget shape without clearing data',
    () async {
      SharedPreferences.setMockInitialValues({});
      final storage = LocalStorageService(
        await SharedPreferences.getInstance(),
      ...
      final backupService = EncryptedBackupService(
        exportService: _StaticExportService(
          ...
          themeRepository: themeRepository,
          hasValidUser: true,
          budget: {'monthlyLimit': 30000, 'categoryLimits': 'Food:9000'},
        ),
        randomBytes: (length) => List<int>.filled(length, 14),
      );
      final backup = await backupService.buildEncryptedBackup(
        password: 'correct horse battery staple',
      );
