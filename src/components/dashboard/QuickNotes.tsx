...
export const QuickNotes: React.FC<{ onNewNote?: () => void }> = ({ onNewNote }) => {
  ...
  return (
    <Card>
      ...
      {recentNotes.map((note, index) => (
        <TouchableOpacity key={note.id} style={{
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={{
                ...
                lineHeight: 16,
              }} numberOfLines={2}>{note.content}</Text>
            </View>
            {note.is_pinned && (
              <MaterialCommunityIcons name="pin" size={14} color={colors.accent} />
            )}
          </View>
          <Text style={{
            fontSize: 10,
            ...
            marginTop: 6,
          }}>{formatRelativeDate(note.updated_at)}</Text>
        </TouchableOpacity>
      ))}
      <TouchableOpacity
        onPress={onNewNote}
