...
- [X] T002 [P] Create the motion-intent policy and reduced-motion helper in `apps/MoneyKai-web/src/utils/motionPolicy.ts` using the `MotionRecipe` rules from `specs/001-calm-intelligent-experience/data-model.md`.
- [X] T003 [P] Create focused unit coverage for motion-intent and reduced-motion fallbacks in `apps/MoneyKai-web/src/utils/motionPolicy.test.ts`.
- [X] T004 [P] Add the experience-state inventory and owner map in `specs/001-calm-intelligent-experience/state-inventory.md` using `WorkspaceSurfaceState` and `SavedReportExportPresentation` from `data-model.md`.
...
- [X] T010 Validate the Phase 2 foundation against `specs/001-calm-intelligent-experience/contracts/ui-experience-contract.md` and record the result in `specs/001-calm-intelligent-experience/validation-log.md`.
